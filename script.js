document.addEventListener("DOMContentLoaded", function () {
    ...
    // 🔹 AI Generate Content Function
    async function generateContent(type) {
        let inputElement = document.getElementById(type);
        if (!inputElement || !inputElement.value.trim()) {
            alert("Please enter some details before AI generation.");
            return;
        }

        const prompt = `Generate a professional ${type} for: ${inputElement.value}`;
        ...
        try {
            ...
            inputElement.value = data?.candidates?.[0]?.content?.parts?.[0]?.text || "No AI suggestions found.";
        } catch (error) {
    ...
    // 🔹 Make generateContent globally accessible
